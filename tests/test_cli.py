...
from bgmi.main import test_main
from bgmi.website.bangumi_moe import BangumiMoe


def test_gen_nginx_conf():
    test_main("gen nginx.conf --server-name _".split())
...
def test_update_single(bangumi_names):
    ...
    test_main(f"add {name}".split())
    test_main(["update", name])


@pytest.mark.usefixtures("_clean_bgmi")
def test_search(bangumi_names):
    test_main(["search", "海贼王", "--regex-filter", ".*MP4.*720P.*"])
...
def test_search_tag(bangumi_names, bangumi_subtitles):
    ...
    test_main(["search", "--tag", "--subtitle", subtitle, name, "--regex-filter", ".*720P.*"])


@pytest.mark.usefixtures("_clean_bgmi")
def test_delete(bangumi_names):
    name = bangumi_names[0]
    test_main(f"add {name} --episode 0".split())
    test_main(f"delete --name {name}".split())


@pytest.mark.usefixtures("_clean_bgmi")
def test_delete_batch(bangumi_names):
    test_main(["add", *bangumi_names, "--episode", "0"])
    test_main("delete  --clear-all --batch".split())
...
def test_filter(bangumi_names):
    name = bangumi_names[0]
    test_main(f"add {name} --episode 0".split())
    test_main(["filter", name, "--subtitle", "", "--exclude", "MKV", "--regex", "720p|720P"])
